...
@media (min-width: 768px) {
  .bio-card {
    display: grid;
    grid-template-columns: 1.7fr 1fr; /* Better balance */
    grid-auto-rows: auto;             /* Prevents blank vertical space */
    gap: 2.5rem;
    align-items: start;
  }
  ...
  .bio-image {
    width: 100%;
    height: auto;
    max-width: 100%;  /* Prevents a fixed 400px column that causes the space */
  }
}
