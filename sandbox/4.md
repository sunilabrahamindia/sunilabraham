...
<section class="lead">
  <p>
    Watch your creation streak bloom! Each flower represents a day of publishing activity.
    Different flowers show different levels of productivity. The garland displays your
    <strong>current uninterrupted streak</strong>, while the timeline below records all activity.
  </p>
</section>
...
if (streakDates.length === 0) {
  ...
  summary.textContent = 
    ...
    `Started ${formatDMY(streakDates[0].date)}`;
}
...
if (streakDates.length === 0) {
  ...
  streakDates.forEach((d, i) => {
    ...
    flower.setAttribute('aria-label', `${formatDMY(d.date)}: ${d.count} page${d.count === 1 ? '' : 's'} created`);
    ...
    info.className = "info";
    info.textContent = `${formatDMY(d.date)}: ${d.count}`;
...
for (let d = startDate; d <= today; d = nextDate(d)) {
  ...
  li.textContent = `${formatDMY(d)}: ${c} ${c === 1 ? "page" : "pages"}`;
