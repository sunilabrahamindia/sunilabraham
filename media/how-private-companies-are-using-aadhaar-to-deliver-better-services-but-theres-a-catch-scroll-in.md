---
layout: default
title: "How Private Companies Are Using Aadhaar to Try to Deliver Better Services (but There's a Catch)"
description: "A Scroll.in report by M. Rajshekhar on private sector adoption of Aadhaar for KYC and data broking, quoting Sunil Abraham on the rise of data aggregation companies."
...
**"How Private Companies Are Using Aadhaar to Try to Deliver Better Services (but There's a Catch)"** is a *Scroll.in* report by M. Rajshekhar published on 22 December 2016. The article examines five ways private companies were incorporating Aadhaar into their systems for authentication, KYC, and database management, with [Sunil Abraham](/sunil/) of the [Centre for Internet and Society](/cis/) warning that the same data infrastructure would enable the rise of commercial data brokers aggregating citizen information from multiple sources.
...
<dl class="media-details">
  ...
  <dd>
    <a class="btn" href="https://scroll.in/article/823274/how-private-companies-are-using-aadhaar-to-deliver-better-services-but-theres-a-catch">
      Read Online
    </a>
  </dd>
</dl>
...
<p>They are gathering more information on you.</p>
...
<p><strong>Aadhaar Enters the Private Sector</strong></p>
...
<p><strong>Five Ways of Using Aadhaar</strong></p>

<p>The first way in which companies are using Aadhaar is <strong>pure authentication.</strong> This is how Authbridge uses Aadhaar. It sends a name and Aadhaar number to the Unique Identity Authority's server, which responds to say whether they have matched.</p>
...
<p>The second is <strong>authentication plus</strong>. Here, at the time of authentication, a company also downloads the customer's data from the Aadhaar database. This is what companies like Reliance Jio are doing.</p>
...
<p>Subsequently, these companies might pad up those databases by adding their own data. This is a third model of using Aadhaar: <strong>authentication plus private database</strong>.</p>

<p>For instance, TrustID, a mobile app which claims it can verify "your maid, driver, electrician, tutor, tenant and all service professionals" using Aadhaar, wants users to rate the services of the people they eventually employ. In effect, it is creating a private database.</p>

<p>Others, like Eko, are adding financial transaction histories to the Aadhaar data.</p>

<p>While these three uses are built around Aadhaar-based authentication, the remaining three uses – <strong>database sharing, data broking, deduplication</strong> – pivot around use of just the Aadhaar number. They are based on recent changes in how companies use customer data.</p>

<p><strong>The Customer Data Boom</strong></p>
...
<p><strong>Enter, the Sharing of Customer Data</strong></p>
...
<p>But a programmer who works at iSpirt, a product software evangelising association based in Bangalore, and who did not want to be identified, said the trend will grow. In the financial sector, as new players like mobile wallet companies acquire more customers, banks that refuse to share data will miss out on emergent markets, he said. "Keeping everything behind closed doors – not participating in data exchanges – is now harmful," he said.</p>
...
<p><strong>The Utility of Aadhaar</strong></p>

<p>Aadhaar makes it easier to <strong>compare and combine diverse databases.</strong></p>

<p>This is what India's microfinance companies are doing. As <em>Scroll.in</em> reported recently, Microfinance Institutions Network, an association of microlenders, has told its member companies to seed the Aadhaar numbers of their borrowers into their databases. By searching the databases for the Aadhaar number of a prospective borrower, it will be possible to identify if she has already taken too many loans.</p>
...
<p>Aadhaar, added Varma, can also be used to clean up databases. Banks, he said, can use the Aadhaar number to create better customer profiles by identifying all accounts owned by a person. This is the fifth use – <strong>deduplication.</strong></p>
...
<p>These algorithms, as Propublica reported, can be opaque.</p>
...
<p>That is where India is heading as well. As Scroll noted in its article about TrustID, when the company creates scores for the workers who use its app, they might not always be aware of that rating – or be in a position to challenge that rating.</p>
...
By late 2016, the public debate on Aadhaar had largely focused on government surveillance and welfare delivery. This article shifted attention to a parallel development that was receiving less scrutiny: the steady incorporation of Aadhaar into private sector systems for identity verification, customer onboarding and database management. The commercial logic was straightforward, since Aadhaar offered a cheap, instant mechanism for KYC compliance at a scale that traditional paper-based processes could not match.

The article identifies five distinct uses, ranging from simple authentication to deduplication across databases, and shows how each use progressively added to the data profile that companies could build on individual citizens. The underlying concern is that Aadhaar's unique identifier function made it far easier to correlate datasets across institutions in ways that would otherwise be technically unreliable.

Sunil Abraham's comment appears in the context of data broking, which is the fourth use the article describes. His argument is prospective rather than reactive: the same infrastructure enabling legitimate data sharing between companies also creates the conditions for a commercial data aggregation industry of the kind that had already emerged in the United States, with limited regulatory oversight and significant potential for misuse.

## External Link

- [Read on Scroll.in](https://scroll.in/article/823274/how-private-companies-are-using-aadhaar-to-deliver-better-services-but-theres-a-catch)
